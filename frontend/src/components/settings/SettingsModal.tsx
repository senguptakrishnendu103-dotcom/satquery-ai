...
export const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose }) => {
  ...
  // Ingestion & Dataset Settings State
  const [localDataPath, setLocalDataPath] = useState<string>(
    () => localStorage.getItem('satquery_local_data_path') || './static/uploads'
  );
  const [maxUploadMb, setMaxUploadMb] = useState<number>(
    () => Number(localStorage.getItem('satquery_max_upload_mb')) || 500
  );
  ...
  const handleSave = () => {
    localStorage.setItem('satquery_local_data_path', localDataPath);
    localStorage.setItem('satquery_max_upload_mb', String(maxUploadMb));
  ...
  const handleReset = () => {
    setLocalDataPath('./static/uploads');
    setMaxUploadMb(500);
      ...
      <div className="relative w-full max-w-3xl max-h-[85vh] flex flex-col rounded-xl border border-sat-border bg-sat-surface/95 shadow-2xl shadow-sat-accent/10 overflow-hidden font-sans">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="font-mono text-[9px] text-sat-dim uppercase tracking-wider">
                Geospatial AI & Remote Sensing Data Ingestion Configurations
              </p>
            </div>
          </div>
        ...
        <div className="flex border-b border-sat-border bg-sat-bg/60 px-6 pt-2">
          ...
          >
            ...
            <span>Data Ingestion & Local Rasters</span>
          </button>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          {/* TAB 1: DATA INGESTION & LOCAL RASTERS */}
          {activeTab === 'providers' && (
            <div className="space-y-5">
              <div className="rounded-lg border border-sat-accent/20 bg-sat-accent/5 p-4 flex items-start gap-3">
                ...
                <div>
                  <h4 className="font-mono text-xs font-bold uppercase text-sat-accent">Local GeoTIFF & Satellite Ingestion</h4>
                  <p className="font-sans text-xs text-sat-muted mt-1 leading-relaxed">
                    SatQuery processes uploaded multi-band GeoTIFFs, ISRO/SAC datasets, and local remote sensing imagery with exact spatial bounds, CRS metadata, and spectral channels.
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div>
                  <label className="block font-mono text-xs font-bold uppercase tracking-wider text-sat-text mb-1.5 flex items-center gap-2">
                    <Database className="h-3.5 w-3.5 text-sat-accent" />
                    Local Raster Ingestion Directory
                  </label>
                  <input
                    type="text"
                    value={localDataPath}
